...
Vagrant.configure("2") do |config|
  ...
  ##### DEFINE VM for chef-server #####
  config.vm.define "lab-chef-server" do |device|
    
    device.vm.hostname = "lab-chef-server" 
    
    device.vm.box = "bento/ubuntu-16.04"
    device.vm.provider "virtualbox" do |v|
      v.name = "#{simid}_lab-chef-server"
      v.customize ["modifyvm", :id, '--audiocontroller', 'AC97', '--audio', 'Null']
      v.memory = 1024
    end
    device.vm.synced_folder ".", "/vagrant", create: true

    # NETWORK INTERFACES
      # link for eth1 --> oob-mgmt-switch:swp1
      device.vm.network "private_network", virtualbox__intnet: "#{simid}_net54", ip: "192.168.0.200", :mac => "443839000060"
      device.vm.network "forwarded_port", guest: 80, host: 8080
      device.vm.network "forwarded_port", guest: 443, host: 8443
    ...
    device.vm.provision :shell , path: "helper_scripts/config_chef_server.sh"

    # Not sure the following section is needed
